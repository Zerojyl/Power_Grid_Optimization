...
def power_process(yaml_path = 'utils/configs/power_forecasting/lstm_power_forecasting.yaml', data = 'data1'):
    ...
    train_dataloader = DataLoader(TensorDataset(X1_train, X2_train, y_train), batch_size=config['model_config']['batch_size'], shuffle=True)
    val_dataloader = DataLoader(TensorDataset(X1_val, X2_val, y_val), batch_size=config['model_config']['batch_size'], shuffle=False)
    return my_dataset, train_dataloader, val_dataloader

if __name__ == '__main__':
    dataset, train_dataloader, val_dataloader = power_process()
    X1_train, X2_train, y_train = next(iter(train_dataloader))
    X1_val, X2_val, y_val = next(iter(val_dataloader))
